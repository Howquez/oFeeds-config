...
raw <- fread(file = "../../dice/studies/meme_feed/data/raw/all_apps_wide_2024-10-15.csv", 
             na.strings = "",
             nrows = 7)
participants_in_raw_data <- raw[, unique(participant.label)]

processed <- fread(file = "../../dice/studies/meme_feed/data/raw/DICE-processed-2024-10-15.csv", 
...
::: {.callout-warning}
## Download your data immediately
The database is reset periodically as part of server maintenance. **Any data not downloaded before a reset is permanently lost.** Do not rely on the server as long-term storage — download your data as soon as data collection is complete and save it in a secure location (e.g., your institution's research storage or a private repository).
:::
...
This data follows oTree's [conventions](https://otree.readthedocs.io/en/latest/conceptual_overview.html#object-hierarchy) as displayed in @tbl-raw.^[In @tbl-raw, we removed a column called `participant.tweets` as it reports all the information displayed in the feed for every participant. We would advise you to delete that column too as it decreases the file size considerably (if you work with long feeds).] Note that data from participants who dropped out mid-study is not lost — their behavioral measures up to the point of drop-off are recorded and included in the download. You can identify incomplete responses by filtering for participants where `participant.finished` is `False`. 
