...
pkgs <- c("magrittr", "data.table", "knitr", "stringr", "english", "moments", "devtools",
          ...
          "multilevelmediation", "mediation", "lme4", "optimx", "psych", "broom.helpers", "broom.mixed")
...
```{r read_brazil_data}
short <- fread(file = "../../dice/studies/brand_safety/data/processed/brand-safety-short.csv", na.strings = "")
long  <- fread(file = "../../dice/studies/brand_safety/data/processed/brand-safety-long.csv", na.strings = "")
...
Each post's content was specified in columns such as `<text>` and `<username>`. We used the `<condition>` parameter to distinguish between our brand-safebrand-unsafe feeds, assigning each row (i.e., each post) to its respective condition.
...
balance_table <- data.table(variables = c("Mean Age (Years)", "Female (Percent)"),
                            safe = c(short[condition == "safe", 
                                                  mean(age, na.rm = TRUE)],
                                            short[condition == "safe",
                                                  mean(female, na.rm = TRUE)]*100),
                            unsafe = c(short[condition == "unsafe", 
                                                  mean(age, na.rm = TRUE)],
                                            short[condition == "unsafe", 
                                                  mean(female, na.rm = TRUE)]*100),
...
```{r read_meme_data}
data <- fread(file = "../../dice/studies/meme_feed/data/processed/meme-feed-data.csv", na.strings = "")
