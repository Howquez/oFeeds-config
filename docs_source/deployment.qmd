...
1. **Python**: oTree requires Python 3.9 or higher. Download and install Python from [python.org](https://www.python.org/downloads/).
...
Now download the `DICE.otreezip` file [(which you can find here)](https://github.com/Howquez/DICE/blob/main/software/DICE/DICE.otreezip) and store it in your new directory. Then use the command prompt or terminal and run the `otree unzip` command to extract the contents of the otreezip file:
