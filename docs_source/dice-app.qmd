...
## Testing vs. Final Configuration

We created this selection for you to create a trimmed down process of launching a study if you only create it for testing purposes. Hence, you can select `Testing` to skip some of the configuration steps while creating a study link for 20 participants.
...
As of today, the DICE app offers two study types, either run a DICE study in your behavioral lab^[You can also use this configuration for other onine recruitment strategies, but we haven't optimized the DICE app correspondingly.] or recruit participants via Prolific. In both cases, you need to specify the number of rows in a database that will be created specifically for your study. Each row can be assumed by one participant. If there is attrition, that is, if a participant enters the study but leaves before finishing it, the row in the database will be taken and cannot be overwritten by other participants. Hence, you should consider attrition while specifying the number of rows in the database as participants who try to enter the experiment after all slots (rows in the database) are used, will receive an error message.

Whereas it is therefore costly for you to underestimate attrition, it is also costly for you (and us) to overestimate attrition and enter an unreasonably large number of desired rows because this can have implications on the performance. Hence, we enforce a maximum of 600 rows here. Feel free to contact us, if you require larger sample sizes and we'll schedule a session together.
