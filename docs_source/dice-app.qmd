...
When you successfully create a session, DICE automatically generates and downloads a JSON configuration file named after the session code (e.g., `abc123_dice_config.json`). This file records all your study settings — including the session code and creation date — and serves two purposes:

- **Reproducibility**: load it to pre-fill the form and quickly recreate or adapt a previous study
- **Debugging & record-keeping**: the session code inside the file lets me look up a past session and could be useful for tracking down issues
...
- Standard sessions support up to **400 slots**
- Sessions of up to **1,000 slots** are available upon authentication with an approved email address and password — contact us if you need access
- For very large samples, consider [deploying](deployment.qmd#sec-deploy) DICE yourself

::: {.callout-important}
## Keep Test Sessions Small
When testing your setup before a real study, please create sessions with only 10–20 slots. Test sessions occupy server resources and database space even if no one participates. Once you are satisfied with the configuration, create a fresh session with the full participant count for your actual study.
:::
...
Select the social media platform style that best fits your study. Each option renders a distinct feed interface:

- **Twitter/X** — Microblogging feed with tweet cards, retweet counts, likes, and replies
- **Instagram** — Image-centric feed with a double-tap to like interaction
- **Stories** — Full-screen vertical stories format; each post is displayed for a (soon-to-be) configurable number of seconds before advancing automatically
- **LinkedIn** — Professional network feed with reaction buttons and connection-style user profiles
- **Generic (Swipe)** — A minimal, platform-agnostic feed that participants swipe through; useful when you want to avoid priming participants with a specific platform association
...
If you prefer using **Google Drive**, share the file by setting it to "Anyone with the link can view." Then modify the sharing URL by replacing `file/d/` with `uc?export=download&id=` and removing everything after the file ID:

```
https://drive.google.com/uc?export=download&id=YOUR_FILE_ID
```

If you prefer using **Google Sheets**, share the sheet by setting it to "Anyone with the link can view" and paste the sharing URL directly into DICE — no modifications needed. DICE will automatically export the sheet as a CSV.
...
Use the **Skip Intro** checkbox to bypass the consent page entirely, and the **Skip Briefing** checkbox to bypass the instructions page. Both are useful during testing or when your study design does not require those pages.
...
The database is reset periodically as part of server maintenance, and **any data not downloaded beforehand is permanently lost**. Once data collection is complete, download your data immediately and store it somewhere safe. Do not treat the server as long-term storage. See the [Data](data.qmd#sec-data) chapter for download instructions.
:::
